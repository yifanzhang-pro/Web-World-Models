      ...
      for (let y = GRID_HEIGHT - 1; y >= 0; y--) {
        ...
        for (let x = xStart; x !== xEnd; x += xStep) {
          ...
          if (enId !== 0) {
              ...
              if (enElem) {
                   // Decay
                   if (Math.random() > (enElem.decayChance || 0)) {
          ...
          if (cellId === hydroId) {
               ...
               if (ignited && fireId) {
                   ...
                   if (fireId) nextEnergyGrid[i] = fireId;
                   continue; 
               }
          }
          ...
          // Default Decay
          if (elem.physics !== PhysicsType.BOT && elem.decayTo !== undefined && Math.random() < (elem.decayChance || 0)) {
          ...
          if (elem.physics === PhysicsType.MECHANISM) {
              ...
              if (cellId === heaterId) {
                   if (fireId && Math.random() < 0.2) {
                       ...
                       if (up !== -1) nextEnergyGrid[up] = fireId;
                   }
                   neighbors.forEach(n => {
                      if (n !== -1 && grid[n] === waterId) {
                           if (nextGrid[n] === waterId) {
                               nextGrid[n] = 0; // Steam/Empty
                               if (Math.random() < 0.3 && fireId) nextEnergyGrid[n] = fireId;
                           }
                      }
                   });
              }
