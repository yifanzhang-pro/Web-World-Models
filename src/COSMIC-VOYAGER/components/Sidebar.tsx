...
const normalizeBodyName = (name: string) => name.replace(/-sel\d+$/, '');
...
const Sidebar: React.FC<SidebarProps> = ({ planet, onClose, onLand, viewContext }) => {
  ...
  useEffect(() => {
    if (planet) {
      const normalizedPlanetName = normalizeBodyName(planet.name);
      const normalizedTargetName = normalizeBodyName(viewContext?.targetName ?? normalizedPlanetName);

      const cached = aiCacheRef.current[normalizedPlanetName];
      setAiContent(cached ?? null);

      const now = Date.now();
      const planetChanged = lastPlanetRef.current !== normalizedPlanetName;
      const modeChanged = lastModeRef.current !== viewContext?.mode;
      const targetChanged = lastTargetRef.current !== normalizedTargetName;
      ...
      if (planetChanged || modeChanged || targetChanged || timedOut) {
        lastPlanetRef.current = normalizedPlanetName;
        lastModeRef.current = viewContext?.mode ?? null;
        lastTargetRef.current = normalizedTargetName;
        lastFetchTimeRef.current = now;
        fetchAiDetails(normalizedPlanetName, planet);
      }
    }
  }, [planet, viewContext?.mode, viewContext?.targetName]);

  const fetchAiDetails = async (name: string, body: PlanetData) => {
    ...
    try {
      ...
      aiCacheRef.current[name] = details;
