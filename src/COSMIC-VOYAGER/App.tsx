...
const App: React.FC = () => {
  ...
  const fetchAutoNarrative = useCallback(async () => {
    ...
    try {
      const details = await getPlanetDetails({
        ...
        bodyType: selectedPlanet?.type,
        fallbackText: selectedPlanet?.description,
  ...
  }, [autoNarrative, narrationTarget, selectedPlanet?.description, selectedPlanet?.type, viewContext]);
